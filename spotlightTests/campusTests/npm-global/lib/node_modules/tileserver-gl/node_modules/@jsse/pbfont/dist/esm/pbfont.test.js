...
function readTestFont(name) {
    const d = fs.readFileSync(path.resolve(__dirname, "..", "test", "fixtures", name));
...
test("compositing two pbfs", (_t) => {
    ...
    if (!composite.stacks[0])
        throw new Error("no composite.stacks[0]");
    expect(composite.stacks[0].name).toBe("Open Sans Regular, Arial Unicode MS Regular");
    ...
    if (recombined) {
        ...
        if (!recomposite)
            throw new Error("no recomposite");
        ...
        if (!recombinedReexpect)
            throw new Error("no recombinedReexpect");
        expect(recomposite.stacks.length).toEqual(reexpect.stacks.length);
        if (!recomposite.stacks[0])
            throw new Error("no recomposite.stacks[0]");
        const recompositelength = recomposite.stacks[0].glyphs.length;
        expect(recompositelength).toEqual(176);
        if (!recomposite.stacks[0])
            throw new Error("no recomposite.stacks[0]");
        if (!reexpect.stacks[0])
            throw new Error("no reexpect.stacks[0]");
        expect(recomposite.stacks[0].glyphs.length).toEqual(reexpect.stacks[0].glyphs.length);
...
test("compositing and providing fontstack string name", (_t) => {
    ...
    if (!composite_name.stacks[0])
        throw new Error("no composite_name.stacks[0]");
    ...
    expect(composite_name.stacks[0].glyphs).toEqual(composite_noname.stacks[0].glyphs);
    expect(composite_name.stacks[0].range).toEqual(composite_noname.stacks[0].range);
...
test("debug method shows decoded glyphs", (_t) => {
    ...
    if (!decodedGlyph)
        throw new Error("no decodedGlyph");
...
//# sourceMappingURL=pbfont.test.js.map